...
class CFR_sys():
    ...
    def plot_draw_TwoBlocks(self):
        ...
        plt.legend()
        plt.show()
    def save_result(self, user_end, path):
        with open(path+'coverage_'+str(user_end), 'wb') as il:
            pickle.dump(self.coverage, il)
        with open(path + 'precision_'+ str(user_end), 'wb') as out:
            pickle.dump(self.precision, out)
        with open(path+'user_'+ str(user_end), 'wb') as out:
    ...
    def log_result(self):
        ...
        with open(path+str(self.movie_id_end)+'_100users.csv', 'ab') as out:
    ...
    def log_mul_users(self, user_end):
        path = './result/top10_large_10w'
        self.save_result(user_end,path)
...
if __name__ == "__main__":
    ...
    # system.inverted_list()
    # load sim matrix
    with open('./sim_matrix_100000') as f:
        system.sim_matrix = pickle.load(f)
    # load inverted list
    with open('./inverted_list_100000') as f1:
        system.inverted_list_dict = pickle.load(f1)

    # for more users at the same topN
    # system.topN = 10
    # system.rec_result(user_end=1000)
    # system.log_mul_users(user_end=1000)
